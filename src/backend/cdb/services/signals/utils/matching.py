...
async def find_existing_signal_record(
    db: AsyncSession,
    signal_id: str,
    ...
    """
    Finds the most recent existing signal record for the exact target entity scope
    across ALL statuses (active, snoozed, acknowledged, actioned, dismissed, resolved)
    to support deduplication, alert suppression, and re-alert evaluation.
    """
    stmt = (
        select(DetectedSignal)
        .where(DetectedSignal.signal_id == signal_id)
        .order_by(DetectedSignal.updated_at.desc())
    )
