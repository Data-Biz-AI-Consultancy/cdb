...
class OpportunityAction(Base, TimestampMixin):
    ...
    )  # 'pipeline', 'deal', 'contacts', 'activity', 'bulk_ops'
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    icon: Mapped[str | None] = mapped_column(String(50), nullable=True)
    color: Mapped[str | None] = mapped_column(String(50), nullable=True)


class OpportunityHistory(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    action_id: Mapped[str] = mapped_column(
        String(50),
        ForeignKey("opportunity_actions.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    ...
    # Relationships
    action: Mapped[OpportunityAction] = relationship("OpportunityAction", lazy="joined")
