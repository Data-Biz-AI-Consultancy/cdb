...
* **Signals Radar & Detail Investigation Web UI (`/signals`, `/signals/[id]`)**: Fast, streamlined triage dashboard featuring a **3-Column Command Center** (`🟢 Opportunities`, `🔴 Risks`, `🟡 Hybrid & Conflicts`) with minimal scannable cards on `/signals` for rapid team triage (`Acknowledge`, `Take Action`, `Dismiss`), dedicated filtering tabs (**All Active**, **⚠️ Conflicting Signals**, **🔍 Needs Verification**, **Signal Dimension Catalog**), and deep-dive investigation pages on `/signals/[id]` presenting full supporting evidence dossiers, multi-entity conflict analysis, uncertainty rationale, and playbook recommendations.
