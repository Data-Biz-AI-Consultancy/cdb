...
* **Signals Radar & Triage Web UI (`/signals`)**: Live interactive dashboard featuring active radar KPI stats, dedicated tabs (**All Active**, **⚠️ Conflicting Signals**, **🔍 Needs Verification**, **Signal Dimension Catalog**), confidence tier badges, conflict alert banners, evidence previews, one-click detection sweep trigger, multi-entity linking tags, and triage actions (`Acknowledge`, `Take Action`, `Dismiss`).
